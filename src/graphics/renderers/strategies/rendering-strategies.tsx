...
// Each renderer variant (canvas, particle) provides its own implementations.

// Typed context passed from IResourceStrategy to IPipelineStrategy so that
// pipeline creation has access to the bind group layouts allocated by the resource strategy.
...
// Owns all GPU resources (buffers, bind groups, layouts). Provides the uniform
// buffer, bind groups for each pass, and the pipeline context for pipeline creation.
// postUpdate() runs after each compute dispatch (e.g. ping-pong buffer swap).
...
// Runs per-frame compute work. Canvas renderers use a no-op implementation.
// Particle renderers dispatch the compute shader and trigger buffer swaps.
...
// Issues the draw call(s) for a frame. Canvas renderers draw a full-screen triangle.
// Particle renderers optionally render a background pass first, then draw instanced quads.
